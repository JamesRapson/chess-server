import { Files, Piece, Position, Ranks } from "./chessGameModel";
import { getPieceAtPosition, getPieceById, isOffBoard } from "./playChessGame";
...
const getKnightPossibleMoves = (pieces: Piece[], knight: Piece): Position[] => {
  const testMoves1 = generatePositionPermutations(
    knight.position,
    [1, -1],
    [2, -2]
  );
  const testMoves2 = generatePositionPermutations(
    knight.position,
    [2, -2],
    [1, -1]
  );
...
const getBishopPossibleMoves = (pieces: Piece[], bishop: Piece): Position[] => {
  ...
  possibleMoves = possibleMoves.concat(
    testMovesAlongDiagonal(pieces, bishop, true, false)
  );
  possibleMoves = possibleMoves.concat(
    testMovesAlongDiagonal(pieces, bishop, false, true)
  );
  possibleMoves = possibleMoves.concat(
    testMovesAlongDiagonal(pieces, bishop, false, false)
  );
  return possibleMoves;
};

const getKingPossibleMoves = (pieces: Piece[], king: Piece): Position[] => {
  const testMoves = generatePositionPermutations(
    king.position,
    [1, 0, -1],
    [1, 0, -1]
  );
...
const getQueenPossibleMoves = (pieces: Piece[], king: Piece): Position[] => {
  ...
  possibleMoves = possibleMoves.concat(
    testMovesAlongDiagonal(pieces, king, true, false)
  );
  possibleMoves = possibleMoves.concat(
    testMovesAlongDiagonal(pieces, king, false, true)
  );
  possibleMoves = possibleMoves.concat(
    testMovesAlongDiagonal(pieces, king, false, false)
  );
...
const testMovesAlongDiagonal = (
  pieces: Piece[],
  piece: Piece,
  rankDirection: boolean,
  fileDirection: boolean
): Position[] => {
  const possibleMoves: Position[] = [];
...
const generatePositionPermutations = (
  startPos: Position,
  rankOptions: number[],
  fileOptions: number[]
): Position[] => {
