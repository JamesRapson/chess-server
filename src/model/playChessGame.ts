...
const canMovePieceToEscapeCheck = (allPieces: Piece[], piece: Piece) => {
  ...
  for (let move of possibleMoves) {
    ...
    if (!isInCheck(game.currentBoard.pieces, piece.player)) return true;
  }
  return false;
};
...
export const isInCheckMate = (pieces: Piece[], player: Player): boolean => {
  ...
  const playersPieces = pieces.filter(
    (piece) => piece.player !== player && !piece.isOffBoard
  );
  for (let piece of playersPieces) {
    if (canMovePieceToEscapeCheck(pieces, piece)) return false;
  }
  return true;
};
...
export const isInCheck = (pieces: Piece[], player: Player): Piece | null => {
  ...
  for (let piece of playersPieces) {
    ...
    if (
      possibleMoves.find(
        (pos) =>
          pos.rank === playersKing.position.rank &&
          pos.file === playersKing.position.file
      )
    ) {
      return piece;
    }
...
export const testIfPossibleMoves = (
  pieces: Piece[],
  player: Player
): boolean => {
  return pieces
...
export const doMove = (game: ChessGame, piece: Piece, position: Position) => {
  ...
  const takenPiece = getPieceAtPosition(
    game.currentBoard.pieces,
    position.rank,
    position.file
  );
